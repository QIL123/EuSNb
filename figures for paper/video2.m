...
pos1=[spacingx*2,spacingy*2+pheight/2,pwidth,pheight];
pos2=[spacingx*3.5+pwidth,spacingy*2+pheight,pwidth,pheight];
pos3=[spacingx*3.5+pwidth,spacingy,pwidth,pheight];
...
for i=1:len
    ...
    %start figure
    fig=figure;
    set(fig,'Units','inches','Position',[0,0,14,8.5])
    ...
    title('B_z^{ac}(x,y) (mT)','Color',fontcolor,'FontSize',fontsize)
        lw=1.5;
    hold on
    annotation("arrow",[0.69,0.69],[0.56,0.6],'Color',anotcolor)
    text(1.8,3.8,max(max(Z)),'x','FontSize',fontsize,'Color',anotcolor);
    annotation("arrow",[0.69,0.715],[0.56,0.56],'Color',anotcolor)
    text(3.8,1.8,max(max(Z)),'y','FontSize',fontsize,'Color',anotcolor);
    %plot DC
    [X,Y,Z]=Proccess_Data.Get_Processed_Data(img1,'DC_(Vfb)',10);
    Z=Z/10;
    ...
    title('B_z^{dc}(x,y) (mT)','FontSize',fontsize,'Color',fontcolor)
    %add anotations
...
set(transport1,'XColor',fontcolor,'YColor',fontcolor,'Color',backcolor,'FontSize',fontsize)
  pause(1)
